...
perdido = Sprite("images/perdeu.png")
perdido.set_position((janela.width - perdido.width) / 2, (janela.height - perdido.height) / 2)
...
def tiro_alien(alvivos):
    x = random.randint(0, alvivos)
    cont = 0
    tiro_al = Sprite("images/tiroalien.png")
    for linha in monstros:
        for monstro in linha:
            if x == cont:
                tiro_al.set_position(monstro.x + monstro.width / 2, monstro.y)
                tiros_alien.append(tiro_al)
            cont += 1


def desenhar_tiros_alien():
    vely = 450
...
def checar_tiros():
    for tiro in tiros:
        for tiro_al in tiros_alien:
            if tiro.collided(tiro_al):
                tiros.remove(tiro)
                tiros_alien.remove(tiro_al)
                break
...
def jogo():
    ...
    while True:
        ...
        for linha in monstros:
            for monstro in linha:
                monstro.x += vel_monstro * janela.delta_time()
                ...
                if bateu_direita == True or bateu_esquerda == True:
                    for linha in monstros:
                        for monstro in linha:
        ...
        pi_x += vel_monstro * janela.delta_time()
        if bateu_direita == True:
            pi_x += 1
        if bateu_esquerda == True:
            pi_x -= 1
        if bt == 1:
